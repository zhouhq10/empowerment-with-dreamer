...
class RSSM(nj.Module):
    ...
    def obs_step(self, prev_state, prev_action, embed, is_first):
        ...
        # _action_clip = 1 -> to ensure action is one-hot encoded?
        if self._action_clip > 0.0:
            prev_action *= sg(
                self._action_clip / jnp.maximum(self._action_clip, jnp.abs(prev_action))
            )
        # If is_first is True, the state and action are zeroed out to prevent the previous episode's 
        # information from influencing the new one.
        ...
        # Model's prior prediction of the next state without using the current observation
        prior = self.img_step(prev_state, prev_action)
        # Combine the prior prediction with the current observation to predict the next state
        x = jnp.concatenate([prior["deter"], embed], -1)
        x = self.get("obs_out", Linear, **self._kw)(x)
        # Construct a distribution over the stochstic latent state
        stats = self._stats("obs_stats", x)
        dist = self.get_dist(stats)
        # Sample from the distribution
        stoch = dist.sample(seed=nj.rng())
        post = {"stoch": stoch, "deter": prior["deter"], **stats}
        # ? difference between post and prior is actually whether you have the observation (embed) or not
...
class MultiEncoder(nj.Module):
    ...
    def __call__(self, data):
        # Accepts a shapes dictionary that defines the shapes of different input data.
        # Seem like that the scalar will be ignored 
...
class MultiDecoder(nj.Module):
    ...
    def __init__(
        ...
        **kw,
        ...
        # certain keys are excluded from decoding.
    ...
    def __call__(self, inputs, drop_loss_indices=None):
        # Generate predictions for both CNN and MLP output
    ...
    def _make_image_dist(self, name, mean):
        ...
        # Creates a Normal distribution with a mean equal to the model output and a standard deviation of 1.
        if self._image_dist == "normal":
            return tfd.Independent(tfd.Normal(mean, 1), 3)
        # Uses a custom Mean Squared Error (MSE)-based "distribution" for loss computation.
        # jaxutils.MSEDist likely implements MSE as a pseudo-distribution, where the "likelihood" corresponds to the squared error.
...
class MLP(nj.Module):
    ...
    def __init__(
        ...
        **kw,
        ...
        if isinstance(shape, int):
            shape = (shape,)
        self._shape = shape
        self._layers = layers # Number of hidden layers in the MLP.
        self._units = units # Number of neurons in each hidden layer.
    ...
    def __call__(self, inputs):
        x = self.get_logits(inputs) # process input through hidden layers
        return self.get_distribution(x) # map them to the desired output format
    ...
    def _out(self, name, shape, x):
        # Applies a distributional transformation to the logits
