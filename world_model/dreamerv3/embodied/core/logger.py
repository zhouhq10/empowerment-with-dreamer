...
class JSONLOutput(AsyncOutput):
    ...
    def __init__(self, logdir, filename="metrics.jsonl", pattern=r".*", parallel=True):
        ...
        # A regular expression to filter which metric names should be logged (.* matches everything by default).
        self._pattern = re.compile(pattern)
    ...
    def _write(self, summaries):
        ...
        lines = "".join(
            [
                json.dumps(
                    {"step": step, "time": time.time() - self._start_time, **scalars}
                )
                + "\n"
...
class TensorBoardOutput(AsyncOutput):
    ...
    def __init__(self, logdir, fps=20, maxsize=1e9, parallel=True):
        ...
        self._fps = fps  # Frames per second for GIF summaries
        self._writer = None
    ...
    def _write(self, summaries):
        ...
        reset = False
        # Check if the current event file is too large
        if self._maxsize:
        ...
        # TensorFlow summary operations need to know where to log data. set_as_default() automatically directs these operations to the right log directory without explicitly passing the writer each time.
        self._writer.set_as_default()

        # Log the summaries
        for step, name, value in summaries:
            try:
                # Scalar, e.g., loss, reward, etc.
                if len(value.shape) == 0:
                    tf.summary.scalar(name, value, step)
                # Histogram, e.g., distributions, weights, biases, etc.
                ...
                # Image, e.g., observation, state, etc.
                elif len(value.shape) == 2:
                    tf.summary.image(name, value, step)
                elif len(value.shape) == 3:
                    tf.summary.image(name, value, step)
                # Video, e.g., rollout, trajectory, etc.
    ...
    def _check(self):
        # Check the size of the current event file
        import tensorflow as tf

        # List all files matching `events.out.*` in the log directory
        events = tf.io.gfile.glob(self._logdir.rstrip("/") + "/events.out.*")
        # Retrieve the size of the most recent event file
        return tf.io.gfile.stat(sorted(events)[-1]).length if events else 0

    def _video_summary(self, name, video, step):
        # Write a video summary (as GIFs) to TensorBoard
        import tensorflow as tf
...
def _encode_gif(frames, fps):
    ...
    # Error: no such filter: 'fifo'
    # The fifo filter is used to buffer frames in filter graphs but may not exist or is deprecated in some builds.
    # cmd = " ".join(
    #     [
    ...
    out, err = proc.communicate()
    if proc.returncode:
        print("FFmpeg Error Output:\n", err.decode("utf8"))  # Added line for debugging
