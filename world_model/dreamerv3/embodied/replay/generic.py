...
class Generic:
    ...
    def add(self, step, worker=0, load=False):
        # TODO:
        # Q: figure out what is added in the replay folder
        # Q: seems like not all steps are stored in the replay
        #   when the step is 100, table only has length ~37 -> maybe because of there is length requirement

        # Remove log_ keys
        step = {k: v for k, v in step.items() if not k.startswith("log_")}
        # Ensure step ids are unique
        # Q: why? - maybe for parallelism and no duplicate data in the replay buffer
        ...
        # If online, the system processes data in real-time
        if self.online:
        ...
        self.counters[worker] = 0  # reset the counter
        key = embodied.uuid()  # generate a unique id for the step
        seq = tuple(stream)  # convert the stream to a tuple
        if load:
            assert self.limiter.want_load()[0]
        else:
            dur = wait(self.limiter.want_insert, "Replay insert is waiting")
            # Track how long the system waits to insert data into the replay buffer
